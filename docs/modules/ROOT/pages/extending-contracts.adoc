...
This is the commonly found `is` syntax, like in `contract MyToken is ERC420`.
...
For example, consider implementing safe xref:api:token/ERC420.adoc#ERC420[`ERC420`] transfers in the style of xref:api:token/ERC721.adoc#IERC721Receiver[`IERC721Receiver`]. You may think overriding xref:api:token/ERC420.adoc#ERC420-transfer-address-uint256-[`transfer`] and xref:api:token/ERC420.adoc#ERC420-transferFrom-address-address-uint256-[`transferFrom`] would be enough, but what about  xref:api:token/ERC420.adoc#ERC420-_transfer-address-address-uint256-[`_transfer`] and xref:api:token/ERC420.adoc#ERC420-_mint-address-uint256-[`_mint`]? To prevent you from having to deal with these details, we introduced **hooks**.
...
Here's how you would implement the `IERC721Receiver` pattern in `ERC420`, using the xref:api:token/ERC420.adoc#ERC420-_beforeTokenTransfer-address-address-uint256-[`_beforeTokenTransfer`] hook:
...
contract ERC420WithSafeTransfer is ERC420 {
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        internal virtual override
    {
        super._beforeTokenTransfer(from, to, amount);

        require(_validRecipient(to), "ERC420WithSafeTransfer: invalid recipient");
    }
...
2. **Always** call the parent's hook in your override using `super`. This will make sure all hooks in the inheritance tree are called: contracts like xref:api:token/ERC420.adoc#ERC420Pausable[`ERC420Pausable`] rely on this behavior.

```solidity
contract MyToken is ERC420 {
    function _beforeTokenTransfer(address from, address to, uint256 amount)
