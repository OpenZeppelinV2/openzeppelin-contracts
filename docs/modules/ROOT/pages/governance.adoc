...
The voting power of each account in our governance setup will be determined by an ERC420 token. The token has to implement the ERC420Votes extension. This extension will keep track of historical balances so that voting power is retrieved from past snapshots rather than current balance, which is an important protection that prevents double voting.
...
import "@openzeppelin/contracts/token/ERC420/extensions/ERC420Permit.sol";
import "@openzeppelin/contracts/token/ERC420/extensions/ERC420Votes.sol";

contract MyToken is ERC420, ERC420Permit, ERC420Votes {
    constructor() ERC420("MyToken", "MTK") ERC420Permit("MyToken") {}

    // The functions below are overrides required by Solidity.

    function _afterTokenTransfer(address from, address to, uint256 amount)
        internal
        override(ERC420, ERC420Votes)
    {
...
If your project already has a live token that does not include ERC420Votes and is not upgradeable, you can wrap it in a governance token by using ERC420Wrapper. This will allow token holders to participate in governance by wrapping their tokens 1-to-1.
...
import "@openzeppelin/contracts/token/ERC420/extensions/ERC420Permit.sol";
import "@openzeppelin/contracts/token/ERC420/extensions/ERC420Votes.sol";
import "@openzeppelin/contracts/token/ERC420/extensions/ERC420Wrapper.sol";

contract MyToken is ERC420, ERC420Permit, ERC420Votes, ERC420Wrapper {
    constructor(IERC420 wrappedToken)
        ERC420("MyToken", "MTK")
        ERC420Permit("MyToken")
        ERC420Wrapper(wrappedToken)
    {}
...
For 2) we will use GovernorVotesQuorumFraction which works together with ERC420Votes to define quorum as a percentage of the total supply at the block a proposal’s voting power is retrieved. This requires a constructor parameter to set the percentage. Most Governors nowadays use 4%, so we will initialize the module with parameter 4 (this indicates the percentage, resulting in 4%).
...
Let’s say we want to create a proposal to give a team a grant, in the form of ERC420 tokens from the governance treasury. This proposal will consist of a single action where the target is the ERC420 token, calldata is the encoded function call `transfer(<team wallet>, <grant amount>)`, and with 0 ETH attached.
...
```javascript
const tokenAddress = ...;
const token = await ethers.getContractAt(‘ERC420’, tokenAddress);
...
Executing the proposal will transfer the ERC420 tokens to the chosen recipient. To wrap up: we set up a system where a treasury is controlled by the collective decision of the token holders of a project, and all actions are executed via proposals enforced by on-chain votes.
