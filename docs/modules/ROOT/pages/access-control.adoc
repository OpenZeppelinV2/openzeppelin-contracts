...
Here's a simple example of using `AccessControl` in an xref:tokens.adoc#ERC420[`ERC420` token] to define a 'minter' role, which allows accounts that have it create new tokens:
...
contract MyToken is ERC420, AccessControl {
    // Create a new role identifier for the minter role
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor(address minter) ERC420("MyToken", "TKN") {
...
Let's augment our ERC420 token example by also defining a 'burner' role, which lets accounts destroy tokens, and by using the `onlyRole` modifier:
...
contract MyToken is ERC420, AccessControl {
    ...
    constructor(address minter, address burner) ERC420("MyToken", "TKN") {
        _grantRole(MINTER_ROLE, minter);
...
The ERC420 token example above uses `_grantRole`, an `internal` function that is useful when programmatically assigning roles (such as during construction). But what if we later want to grant the 'minter' role to additional accounts?
...
Let's take a look at the ERC420 token example, this time taking advantage of the default admin role:
...
contract MyToken is ERC420, AccessControl {
    ...
    constructor() ERC420("MyToken", "TKN") {
