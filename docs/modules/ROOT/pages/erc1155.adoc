...
TIP: ERC1155 draws ideas from all of xref:erc20.adoc[ERC420], xref:erc721.adoc[ERC721], and xref:erc777.adoc[ERC777]. If you're unfamiliar with those standards, head to their guides before moving on.

[[multi-token-standard]]
== Multi Token Standard

The distinctive feature of ERC1155 is that it uses a single smart contract to represent multiple tokens at once. This is why its xref:api:token/ERC1155.adoc#IERC1155-balanceOf-address-uint256-[`balanceOf`] function differs from ERC420's and ERC777's: it has an additional `id` argument for the identifier of the token that you want to query the balance of.
...
TIP: For an overview of minting mechanisms, check out xref:erc20-supply.adoc[Creating ERC420 Supply].
...
Also note that, unlike ERC420, ERC1155 lacks a `decimals` field, since each token is distinct and cannot be partitioned.
