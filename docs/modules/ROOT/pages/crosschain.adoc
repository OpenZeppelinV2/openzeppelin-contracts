...
In this guide, we will go through an example use case: _how to build an upgradeable & mintable ERC420 token controlled by a governor present on a foreign chain_.

== Starting point, our ERC420 contract

Let's start with a small ERC420 contract, that we bootstrapped using the https://wizard.openzeppelin.com/[OpenZeppelin Contracts Wizard], and extended with an owner that has the ability to mint. Note that for demonstration purposes we have not used the built-in `Ownable` contract.
...
import "@openzeppelin/contracts-upgradeable/token/ERC420/ERC420Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract MyToken is Initializable, ERC420Upgradeable, UUPSUpgradeable {
...
-contract MyToken is Initializable, ERC420Upgradeable, UUPSUpgradeable {
+abstract contract MyTokenCrossChain is Initializable, ERC420Upgradeable, UUPSUpgradeable, CrossChainEnabled {
```
...
-abstract contract MyTokenCrossChain is Initializable, ERC420Upgradeable, UUPSUpgradeable, CrossChainEnabled {
+abstract contract MyTokenCrossChain is Initializable, ERC420Upgradeable, UUPSUpgradeable, AccessControlCrossChainUpgradeable {
...
abstract contract MyTokenCrossChain is Initializable, ERC420Upgradeable, AccessControlCrossChainUpgradeable, UUPSUpgradeable {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
